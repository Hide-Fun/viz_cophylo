...
-   `geom_tiplab(align = TRUE)`: tip ラベルを整列表示します。
-   `geom_treescale()`: 枝長スケールを図に追加します。
-   `geom_nodelab()`: 内部ノード bootstrap 値を表示します。
...
-   `.tiplab_segment_data()`: align 用の補助線座標を作ります。
-   `.tiplab_marquee_data()`: `geom_marquee()` 用 tip 座標を作ります。
-   `geom_marquee()`: tip ラベルを同一フォントで安定描画します。
